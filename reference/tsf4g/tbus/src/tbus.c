...
void tbus_send_end(tbus_t* tb, tbus_atomic_size_t len) {
  ...
  // 0 is invalid ("empty package"), so cannot send 0 bytes.
  if (len == 0) {
    return;
  }
...
static char* peek(char* start, char* limit, struct iovec* iov, size_t* iov_num) {
  ...
  for (ch = start; ch < limit;) {
    ...
    if (limit - ch < (ptrdiff_t)sizeof(tbus_header_t)) {
      ret = limit;
      goto done;
    }
    ...
    if (limit - ch < (ptrdiff_t)(sizeof(tbus_header_t) + header->size)) {
      ret = NULL;
      goto done;
    }
