...
// key_t lives in sys/ipc.h on Linux.
#include <sys/ipc.h>
...
#define tbus_size(packet_size, packet_num) \
  (TLIBC_OFFSET_OF(tbus_t, buff) + ((packet_size) + sizeof(tbus_header_t)) * (packet_num))

#define tbus_packet2offset(tb, ptr) \
  ((tbus_atomic_size_t)((const char*)(ptr) - (tb)->buff - (long)sizeof(tbus_header_t)))
