...
// Source: reference/disruptor/src/main/java/com/lmax/disruptor/TimeoutBlockingWaitStrategy.java
...
public:
  // Java takes (timeout, TimeUnit). C++ port takes an already-converted nanoseconds value.
  explicit TimeoutBlockingWaitStrategy(int64_t timeoutInNanos) : timeoutInNanos_(timeoutInNanos) {}
  ...
  int64_t waitFor(int64_t sequence,
    ...
    if (cursorSequence.get() < sequence) {
      ...
      while (cursorSequence.get() < sequence) {
        barrier.checkAlert();
        timeoutNanos = disruptor::util::Util::awaitNanos(cv_, lock, timeoutNanos);
        if (timeoutNanos <= 0) {
